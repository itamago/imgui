...
namespace ImGui
{
    ...
    IMGUI_API void          TableHeader(const char* label, bool draw=true);     // submit one header cell manually (rarely used)
...
enum ImGuiStyleVar_
{
    ...
    ImGuiStyleVar_MenuBarMinHeight,         // float     MenuBarMinHeight
    ImGuiStyleVar_COUNT
};
...
struct ImGuiStyle
{
    ...
    float       MenuBarMinHeight;           // Minimum height of a menu bar. 0.0f = use default (FontSize + FramePadding.y * 2). (default 0.0f)
...
static inline bool   operator!=(const ImVec2& lhs, const ImVec2& rhs)           { return (lhs.x != rhs.x) || (lhs.y != rhs.y); }
static inline ImVec2 operator -(const ImVec2 & v)                               { return ImVec2(-v.x, -v.y); }
...
static inline ImVec4 operator*(const float lhs, const ImVec4& rhs)              { return ImVec4(rhs.x * lhs, rhs.y * lhs, rhs.z * lhs, rhs.w * lhs); }
static inline ImVec4 operator/(const ImVec4& lhs, const float rhs)              { return ImVec4(lhs.x / rhs, lhs.y / rhs, lhs.z / rhs, lhs.w / rhs); }
static inline ImVec4 operator/(const float lhs, const ImVec4& rhs)              { return ImVec4(rhs.x / lhs, rhs.y / lhs, rhs.z / lhs, rhs.w / lhs); }
IM_MSVC_RUNTIME_CHECKS_RESTORE
#endif
...
#ifdef IM_COLOR_CLASS_EXTRA
    IM_COLOR_CLASS_EXTRA     // Define additional constructors and implicit cast operators in imconfig.h to convert back and forth between your math types and ImVec4.
#endif
